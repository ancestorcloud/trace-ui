...
const sizing = {
  big: css({ height: pxToEm(48), width: pxToEm(194), fontSize: pxToEm(14) }),
  small: css({ height: pxToEm(40), width: pxToEm(138), fontSize: pxToEm(13) })
}

const createTheme = ({ primary, light, dark }) =>
  css({
    color: colors.white,
...
const Button = ({ theme: th = 'primary', small, children, ...rest }) => (
  <button
    {...transformProps(whiteListedProps, rest)}
    {...normalize}
    {...baseStyle}
    {...theme[th]}
    {...sizing[small ? 'small' : 'big']}
  >
    {children}
  </button>
)
