...
const Modal = ({
  ...
  const Alt = <Button theme='text' onClick={onAltClick || onRequestClose}>{altCopy}</Button>
  const Primary =
    <Button onClick={onPrimaryClick} theme={theme === 'alert' ? 'alert' : 'primary'}>
      {primaryCopy}
    </Button>
  const Artwork = Aw || artwork[theme]

  return (
    <Base isOpen onRequestClose={onRequestClose} width={pxToEm(572)} maxWidth={pxToEm(572)}>
...
const topRight = css({
  ...
  right: 10,
  top: 15,
  zIndex: 1 // otherwise it get's placed behind the artwork
})
